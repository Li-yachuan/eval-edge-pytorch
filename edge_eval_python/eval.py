...
def need_test(root, dset,light):
    flag = False
    sub_pth = os.listdir(root)
    flag_dir = "nms-eval-9" if light else "nms-eval"
...
def getFlist(args):
    ...
    for file_dir in file_dirs:
        for root, _, _ in os.walk(file_dir):
            if need_test(root, args.dataset,args.light):
                dirs_.add(root)
                print(root)
    print("*" * 40+"\n")
    return dirs_
...
class Parser_One_Epoch(object):
    def __init__(self,root,dataset,light):
        self.root = root
    ...
    while True:
        ...
        if len(fset) != 0:
            for updataset in fset:
                parser_one_epoch = Parser_One_Epoch(updataset,args.dataset,args.light)
