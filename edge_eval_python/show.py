...
def getFlist(dir_str, light):
    dirs_ = set()
    file_dirs = dir_str.split(' ')
    for file_dir in file_dirs:
        for root, dirs, files in os.walk(file_dir):
            if light and root.split("/")[-1] == "nms-eval-9" and 'eval_bdry.txt' in files:
                    dirs_.add(os.path.join(root, 'eval_bdry.txt'))
            elif not light and root.split("/")[-1] == "nms-eval" and 'eval_bdry.txt' in files:
                    dirs_.add(os.path.join(root, 'eval_bdry.txt'))
...
def main(args):
    ...
    for txt_path in getFlist(args.dir, args.light):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-l", "--light", action="store_true",
                        help="if true, show the result of light test")
